...
(define-public (contribute-to-pool (amount uint))
  (begin
    ...
    (map-set lender-contributions
      ...
      (merge 
        (default-to 
          { total-contributed: u0, total-earned: u0, active-contribution: u0 }
          (map-get? lender-contributions { lender: tx-sender })
        )
        { 
          total-contributed: (+ amount (get total-contributed (default-to { total-contributed: u0, total-earned: u0, active-contribution: u0 } (map-get? lender-contributions { lender: tx-sender })))),
          active-contribution: (+ amount (get active-contribution (default-to { total-contributed: u0, total-earned: u0, active-contribution: u0 } (map-get? lender-contributions { lender: tx-sender }))))
        }
      )
    )
    (ok amount)
...
(define-public (apply-for-loan (amount uint) (purpose (string-ascii 500)))
  (begin
    (asserts! (and (>= amount MIN_LOAN_AMOUNT) (<= amount MAX_LOAN_AMOUNT)) ERR_INVALID_AMOUNT)
    (asserts! (is-none (map-get? loan-applications { applicant: tx-sender })) ERR_LOAN_ALREADY_ACTIVE)
...
(define-public (approve-loan (applicant principal))
  (let 
    (
      (application (unwrap! (map-get? loan-applications { applicant: applicant }) ERR_LOAN_NOT_FOUND))
      (loan-amount (get amount application))
      (interest-amount (/ (* loan-amount INTEREST_RATE) u100))
    ...
    (map-set loan-applications
      { applicant: applicant }
      (merge application { approved: true, loan-id: (some loan-id) })
    )
  ...
  (let 
    ...
    (map-set borrower-stats
      ...
      (merge 
        ...
        { 
          total-loans: (+ u1 (get total-loans (default-to { total-loans: u0, total-repaid: u0, current-loans: u0, reputation-score: u100 } (map-get? borrower-stats { borrower: borrower })))),
          current-loans: (+ u1 (get current-loans (default-to { total-loans: u0, total-repaid: u0, current-loans: u0, reputation-score: u100 } (map-get? borrower-stats { borrower: borrower }))))
        }
      )
    )
...
(define-public (make-payment (loan-id uint) (amount uint))
  (let 
    ...
    (let ((final-status (if (is-eq new-repaid total-due) "repaid" "disbursed")))
      (map-set loans
        { loan-id: loan-id }
        (merge loan { 
        ...
        (begin
          ...
          (map-set borrower-stats
            ...
            (merge 
              ...
              { 
                total-repaid: (+ total-due (get total-repaid (default-to { total-loans: u0, total-repaid: u0, current-loans: u0, reputation-score: u100 } (map-get? borrower-stats { borrower: borrower })))),
                ...
                reputation-score: (if (< (+ (get reputation-score (default-to { total-loans: u0, total-repaid: u0, current-loans: u0, reputation-score: u100 } (map-get? borrower-stats { borrower: borrower }))) u50) u1000) (+ (get reputation-score (default-to { total-loans: u0, total-repaid: u0, current-loans: u0, reputation-score: u100 } (map-get? borrower-stats { borrower: borrower }))) u50) u1000)
              }
            )
          )
          (ok new-repaid)
  ...
  (let 
    ...
    (map-set borrower-stats
      ...
      (merge 
        ...
        { 
          reputation-score: (if (> (get reputation-score (default-to { total-loans: u0, total-repaid: u0, current-loans: u0, reputation-score: u100 } (map-get? borrower-stats { borrower: borrower }))) u30) (- (get reputation-score (default-to { total-loans: u0, total-repaid: u0, current-loans: u0, reputation-score: u100 } (map-get? borrower-stats { borrower: borrower }))) u30) u0)
        }
      )
    )
    (ok loan-id)
...
(define-public (update-reputation (borrower principal) (score uint))
  (begin
    (asserts! (is-eq tx-sender CONTRACT_OWNER) ERR_UNAUTHORIZED)
...
(define-read-only (get-available-funds)
  (let 
    ...
    (ok (if (> pool-balance reserve-amount) (- pool-balance reserve-amount) u0))
  )
)

(define-read-only (can-borrow (applicant principal) (amount uint))
  (let 
    (
      (stats (default-to 
                { total-loans: u0, total-repaid: u0, current-loans: u0, reputation-score: u100 }
    ...
    loan-data 
      (let 
        ...
        (ok {
          ...
          minimum-payment: (if (> remaining u0) (if (> (/ remaining u10) u10000) (/ remaining u10) u10000) u0),
...
(define-public (distribute-interest-to-lender (lender principal) (amount uint))
  (begin
    (asserts! (is-eq tx-sender CONTRACT_OWNER) ERR_UNAUTHORIZED)
    (asserts! (> amount u0) ERR_INVALID_AMOUNT)
    (asserts! (>= (var-get total-pool) amount) ERR_INSUFFICIENT_FUNDS)
    ...
    (map-set lender-contributions
      ...
      (merge 
        ...
        { 
          total-earned: (+ amount (get total-earned (default-to { total-contributed: u0, total-earned: u0, active-contribution: u0 } (map-get? lender-contributions { lender: lender }))))
        }
      )
    )
    (ok amount)
